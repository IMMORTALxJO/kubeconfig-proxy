...
The script runs `make check`, creates or reuses local kind clusters named `kubeconfig-proxy-a` and `kubeconfig-proxy-b`, builds a temporary kubeconfig, adds proxy contexts using `bin/kubeconfig-proxy`, and validates real Kubernetes API behavior through `kubectl`.
...
- Two kind clusters: `kubeconfig-proxy-a` and `kubeconfig-proxy-b` with contexts `kind-kubeconfig-proxy-a` and `kind-kubeconfig-proxy-b`.
